#!/bin/bash
# Generate multiple days of cyber intel activity
set -euo pipefail
...
for i in {1..7}; do
  PAST_DATE=$(date -v-${i}d +%Y-%m-%d 2>/dev/null || date -d "${i} days ago" +%Y-%m-%d)
  echo "Generating activity for ${PAST_DATE}..."

  # Create custom script for each date
  cat > "temp-${i}.sh" << EOF
#!/bin/bash
set -euo pipefail
...
DevSecOps recommendations included.
...
echo "Pushing all activities to GitHub..."
git push origin main

echo "✅ Generated 7 days of cyber intelligence activity!"